...
@description('The SQL prompt template for generating dynamic T-SQL queries based on user input and table schema within a function.')
param functionAppSqlPrompt string = '''A valid T-SQL query to find {query} for tables and columns provided below:
1. Table: Clients
...
@description('The system message guiding the assistant to provide accurate summaries and counts of call transcripts by ClientId within a function.')
param functionAppCallTranscriptSystemPrompt string = '''You are an assistant who provides wealth advisors with helpful information to prepare for client meetings.
...
@description('The system message guiding the assistant to provide accurate client meeting summaries with correct names and timestamps within a function.')
param functionAppStreamTextSystemPrompt string = '''
...
module azureFunctions 'deploy_azure_function_script.bicep' = {
  ...
  params:{
    ...
    functionAppSqlSystemMessage: functionAppSqlPrompt
    functionAppCallTranscriptSystemMessage: functionAppCallTranscriptSystemPrompt
    functionAppStreamTextSystemMessage: functionAppStreamTextSystemPrompt
  }
  dependsOn:[storageAccountModule]
}
